...
Já que estamos visualizando quantos filmes por mês e por dia foram lançados, vamos aproveitar e verificar qaunto foi o total de lucro agrupando por mês e por dia os dados. Para este caso iremos utilizar filtrar apenas os dados onde a moeda da receita e do orçamento são dolares (US$).
...
imdb %>% 
  ...
  dplyr::select(Mes, Lucro_por_dia) %>% 
  dplyr::filter(!is.na(Mes)) %>% 
  dplyr::arrange(desc(Lucro_por_dia)) %>% 
  knitr::kable()

```

Podemos ver a nuvens de palavras com os meses mais e menos lucrativos.
...
Tabela com os dias e os respectivos lucros totais.
