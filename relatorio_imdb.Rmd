...
E também como sendo praticamente a moeda majoritária quando se trata de receita dos filmes. Na tabela abaixo podemos ver que o dollar representa 99,80% da receita dos filmes.
...
imdb %>%
  ...
  dplyr::arrange(desc(n)) %>% 
  dplyr::rename(
    Moeda_receita = moeda_receita,
    Qte_Filmes = n
  ) %>% 
  dplyr::mutate(
    Porcentagem = Qte_Filmes / sum(Qte_Filmes) * 100
  ) %>% 
  knitr::kable()
```
...
imdb %>%
  ...
  ggplot2::ggplot(ggplot2::aes(x = forcats::fct_reorder(genero_1, total_lucro), y = total_lucro)) +
  ggplot2::geom_col(fill = "#D8BFD8") + 
  # ggplot2::geom_text(ggplot2::aes(
  #   label = paste0(round(total_lucro/1000000000, 1), " B")),
  #   color = "#000000",
  #   hjust = 1) +
  ggplot2::theme_bw() +
  ggplot2::labs(
...
Logo abaixo mostramos a relação entre orçamento, receita e lucro dos filmes com orçamento e receita em dollares. Vamos avaliar se os filmes com maiores orçamentos são os com maiores lucros.
...
ggplot2::ggplot(g1, ggplot2::aes(x = total_orcamento, y = total_receita, color = genero_1)) +
  ...
  ggrepel::geom_label_repel(
    data = subset(g1, total_lucro > 3000000000),
...
Aqui temos a data de nascimento e a sua idade atual.
...
ggplot2::ggplot(
  ...
  ggplot2::geom_text(
    x = tmp$titulo[10], 
    y = mean(tmp$lucro) - 5e+07, 
    label = "USD 323.706.161",
    color = "#000000",
    size = 3.5,
  ) +
  ggplot2::geom_text(
